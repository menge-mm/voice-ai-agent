...
import { ArrowRight, StopCircle, Paperclip, FileText, X, Globe, Volume2, Mic, MicOff } from 'lucide-react'
...
export function MessageInput({ conversationId }: MessageInputProps) {
  ...
  const isStreaming = streamState?.isStreaming && streamState.conversationId === conversationId
  ...
  const [, submitAction, isPending] = useActionState(
    async (_prevState: SendMessageState, formData: FormData): Promise<SendMessageState> => {
  ...
  return (
    <div
      className="fixed bottom-0 right-0 z-10 p-2 transition-all duration-300 pointer-events-none"
                  ...
                  <div className="absolute left-3 right-3 bottom-3 flex items-center justify-between w-[calc(100%-24px)]">
                    ...
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      ...
                      <button
                        ...
                        title={ttsEnabled ? "TTS enabled - AI will speak responses" : "Enable TTS"}
                      >
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          (!whisperReady || isDisabled) && "opacity-50 cursor-not-allowed"
                        )}
                        ...
                        <div className="flex items-center gap-1 ml-1 max-w-[120px] overflow-hidden">
                          {attachments.slice(0, 2).map((file, i) => {
                            ...
                            if (isImage) {
                              ...
                              return (
                                <div key={`${file.name}-${i}`} className="group relative h-5 w-5 overflow-hidden rounded border border-black/20 dark:border-white/20 shadow-sm shrink-0">
                                  <img
                                    src={url}
                                    alt={file.name}
                            ...
                            return (
                              <div key={`${file.name}-${i}`} className="group relative h-5 w-5 rounded border border-black/20 dark:border-white/20 bg-black/5 dark:bg-white/5 flex items-center justify-center shadow-sm shrink-0">
